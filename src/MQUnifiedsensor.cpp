...
MQUnifiedsensor::MQUnifiedsensor(String Placa,
                                 float Voltage_Resolution,
                                 int ADC_Bit_Resolution,
                                 int pin,
                                 String type) {
  this->_pin = pin;
...
void MQUnifiedsensor::setADC(int value) {
  this->_sensor_volt =
      (value)*_VOLT_RESOLUTION / ((pow(2, _ADC_Bit_Resolution)) - 1);
  this->_adc = value;
}
...
void MQUnifiedsensor::serialDebug(bool onSetup) {
  if (onSetup) {
    ...
    Serial.println(
        "Note: remember that all the parameters below can be modified during "
        "the program execution with the methods:");
    Serial.println(
        "setR0, setRL, setA, setB where you will have to send as parameter the "
        "new value, example: mySensor.setR0(20); //R0 = 20KΩ");

    Serial.println(
        "Authors: Miguel A. Califa U - Yersson R. Carrillo A - Ghiordy F. "
        "Contreras C");
    Serial.println(
        "Contributors: Andres A. Martinez - Juan A. Rodríguez - Mario A. "
        "Rodríguez O ");
    ...
    if (!_firstFlag) {
      ...
      Serial.println(
          ...
          "*|");
      Serial.println(
          "|ADC_In | Equation_V_ADC | Voltage_ADC |        Equation_RS        "
          "| Resistance_RS  |    EQ_Ratio  | Ratio (RS/R0) | Equation_PPM |    "
          " PPM    |");
...
float MQUnifiedsensor::validateEcuation(float ratioInput) {
  ...
  if (_regressionMethod == 1)
    ...
    double ppm_log =
        (log10(ratioInput) - _b) /
        _a;  // Get ppm value in linear scale according to the the ratio value
...
float MQUnifiedsensor::readSensor() {
  // More explained in:
  // https://jayconsystems.com/blog/understanding-a-gas-sensor
  _RS_Calc = ((_VOLT_RESOLUTION * _RL) / _sensor_volt) -
             _RL;  // Get value of RS in a gas
  if (_RS_Calc < 0)
    _RS_Calc = 0;                 // No negative values accepted.
  _ratio = _RS_Calc / this->_R0;  // Get ratio RS_gas/RS_air
  if (_ratio <= 0)
    _ratio = 0;  // No negative values accepted or upper datasheet
                 // recomendation.
  if (_regressionMethod == 1)
    _PPM =
        _a *
        pow(_ratio,
            _b);  // <- Source excel analisis
  ...
  // if(_PPM > 10000) _PPM = 99999999; //No negative values accepted or upper
  // datasheet recomendation.
  return _PPM;
}
float MQUnifiedsensor::readSensorR0Rs() {
  ...
  if (_RS_Calc < 0)
    _RS_Calc = 0;  // No negative values accepted.
  _ratio = this->_R0 /
           _RS_Calc;  // Get ratio RS_air/RS_gas <- INVERTED for MQ-131 issue 28
...
float MQUnifiedsensor::calibrate(float ratioInCleanAir) {
  ...
  */
  ...
  RS_air = ((_VOLT_RESOLUTION * _RL) / _sensor_volt) -
           _RL;  // Calculate RS in fresh air
  if (RS_air < 0)
...
float MQUnifiedsensor::getVoltage(int read) {
  ...
  if (read) {
    ...
    voltage = (avg / retries) * _VOLT_RESOLUTION /
              ((pow(2, _ADC_Bit_Resolution)) - 1);
  } else {
